...
#ifdef HYPER_BBS
    outs (HB_BACK"[200;442;507m[¡÷]¾\\Åª [200;442;616m[^P]µoªí¤å³¹ [200;442;722m[z]ºëµØ°Ï [200;442;609m[TAB]¤åºK [200;442;621m[^U]¤W½uªÌ [200;442;626m[^Z][200;442;500;612m[PgUp[201m/[200;442;632;612mPgDn][201m\n" COLOR1 "[1m  ½s¸¹      ");
#else
    outs ("\
    ...
    fileheader *ent;
{
	...
	if ( ent->score > 0 ) {
		...
		sprintf(buf , "%s%s %s",
			"\033[1;31m" , "Æg" , colors[(unsigned int) (ent->date[4] + ent->date[5]) % 7]);    
	    }
	}
	else {
	    if ( ent->score > -9 ) {
		...
		sprintf(buf , "%s%s %s",
			"\033[1;32m" , "Äê" , colors[(unsigned int) (ent->date[4] + ent->date[5]) % 7]);  
	    }
	}
    }
    else
...
#if 1   /* itoc.001203: ¥[±Kªº¤å³¹«e­±¥X²{ X ¦r¼Ë */
...
do_post ()
{
	...
	if (!(currbrdattr & BRD_NOCOUNT))
	{
		...
		if (money >= 100)
		{
			...
			fprintf (fp, "¼ÐÃD: %s\n®É¶¡: %s\n", "POSTÀË¬d", ctime (&now));
			fprintf (fp,
				"%s µoªí¤@½g %d ¦rªº¤å³¹©ó %s ªO\nªá¤F %d ¬í¡A±o¨ìª÷¹ô %d ¤¸,¸gÅç­È %d ÂI"
    ...
    char *direct;
{
    ...
    /* itoc.001203: ¥[±Kªº¤å³¹¥u¦³­ì§@ªÌ¡B¥i¨£¦W³æ¥H¤ÎªO¥D¯à¾\Åª */
    if (fhdr->filemode & FILE_REFUSE && !(currmode & MODE_BOARD) &&
    ...
    char *direct;
{
	...
	if (getdata (b_lines - 1, 0, "¼ÐÃD¡G", genbuf, TTLEN, EDIT, tmpfhdr.title))
	{
    ...
    char *direct;
{
	...
	if (fhdr->filemode & FILE_MARKED)
	{
	    ...
	    //      if (!(currbrdattr & BRD_NOCOUNT)) /* ¤£¦C¤J²Î­pªºª© */
	    //      {
    ...
    char *direct;
{
    ...
    if(!HAS_PERM(PERM_SYSOP)){					//¤£¬O¯¸ªø©Î
	if(!(currmode & MODE_BOARD)){			//¤£¬Oª©¥D©Î
	    if (currstat != READING || (!(CompareId == 0))) //¤£¬O¦Û¤vªº¤å³¹±µ¤£¥i¥[±K
		return RC_NONE;
	    //pressanykey ("No!!!");
	}
    }                                                                              
    ...
    char *direct;
{
    ...
    void
score_note(prompt, fhdr, direct, choose, continu)        //±ÀÂË¥[¤À!! ¾Ç ptt by hialan
    fileheader * fhdr;
    char *direct, *prompt;
    int choose ;
{
	...
	if ( choose == 1 ) {
	    ...
	    pressanykey("¦R¼Ñ¥¢±Ñ!!ÀÉ®×¦³°ÝÃD©Î¦³¤H¥¿¦b¦R¼Ñ, ½Ð­«·s¦R¼Ñ:)");
	    return;
	}
    }
    ...
    if ( choose == 1 ) {
	if ( continu == 0 ) {                                                                           
	    fprintf(fp, "\n\033[1;31m¡÷ \033[33m%s\033[0;33m:%-*s\033[1;31m±À \033[m%14.14s %0d/%0d",
		    cuser.userid, 51-strlen(cuser.userid), genbuf, fromhost,
		    ptime->tm_mon+1, ptime->tm_mday);
	}

	else {
	    fprintf(fp, "\n\033[1;31m¸É \033[33m%s\033[0;33m:%-*s\033[1;31m±À \033[m%14.14s %0d/%0d",
		    cuser.userid, 51-strlen(cuser.userid), genbuf, fromhost,
		    ptime->tm_mon+1, ptime->tm_mday);
	}
    }
	...
	if ( continu == 0 ) {
	    ...
	    fprintf(fp, "\n\033[1;32m¸É \033[33m%s\033[0;33m:%-*s\033[1;32m¦R \033[m%14.14s %0d/%0d",
		    cuser.userid, 51-strlen(cuser.userid), genbuf, fromhost,
		    ptime->tm_mon+1, ptime->tm_mday);
	}
    }  
    ...
    char *direct;
{
    ...
    {
	pressanykey("¥»ªO¤£¶}©ñµû¤À¡I");
	return RC_NONE;
    }                                                                                   

    getdata (b_lines, 0, "½Ð°Ý­n [1]¥[¤À [2]¦©¤À [3]±ÀÂË [4]¦R¼Ñ [5]¸É¥R¯d¨¥ : ", buf, 2, LCECHO, 0);
	...
	if ((HAS_PERM(PERM_SYSOP)) || (CompareId != 0)){
	    if ( fhdr->score == 9 ) {
		sprintf(buf , "¤w¸g¬O³Ì%s¤À¤F!!", fhdr->score >= 9 ? "°ª" : "§C");
		pressanykey(buf);
		return RC_DRAW;
	    }
		...
		if ( HAS_PERM(PERM_SYSOP)) {
		    ...
		    getdata (b_lines, 0, "½Ð°Ý­n¥[´X¤À : ", needscore, 4, LCECHO, 0);
		    fhdr->score += atoi(needscore) ;
		    if ( fhdr->score > 9 )
			fhdr->score = 9 ;
		    score_note("±ÀÂË¤º®e(¤£¿é¤J¸õ¹L): ", fhdr, direct, 1, 0);
	...
	if ((HAS_PERM(PERM_SYSOP)) || (CompareId !=0)){
		...
		if ( HAS_PERM(PERM_SYSOP)) {
		    ...
		    if ( fhdr->score < -9 )
			fhdr->score = -9 ;
		    score_note("¦R¼Ñ¤º®e(¤£¿é¤J¸õ¹L): ", fhdr, direct, 2, 0);
    ...
    else if(buf[0] == '3')        // ±ÀÂË ....:) ¾Ç ptt by hialan
    {
	score_note("±ÀÂË¤º®e: ", fhdr, direct, 1, 0);
	return RC_DRAW;
    }

    else if(buf[0] == '4')        // ±ÀÂË ....:) ¾Ç ptt by hialan
    {
	score_note("¦R¼Ñ¤º®e: ", fhdr, direct, 2, 0);
	return RC_DRAW;
    }

    else if(buf[0] == '5')
    {
	getdata (b_lines, 0, "½Ð°Ý­n [1]±ÀÂË¸É¥R [2]¦R¹Ë¸É¥R : ", buf, 2, LCECHO, 0);
	if ( buf[0] == '1' ) {
	    score_note("¸É±À¤º®e(¤£¿é¤J¸õ¹L): ", fhdr, direct, 1, 1) ;
	    return RC_DRAW;
	}

	else if ( buf[0] == '2' ) {
	    score_note("¸É¦R¤º®e(¤£¿é¤J¸õ¹L): ", fhdr, direct, 2, 1) ;
	    return RC_DRAW;
	}	
    ...
    if(!HAS_PERM(PERM_SYSOP) && (CompareId !=0))                   // ¯¸ªø¤£¦©
    {                                                              // ¦¸¼Æ¦ý¬O
	//ingold(1);                                                   // ¤£¥[¿ú
	cuser.scoretimes--;
    }
	...
	sprintf(buf , "%-12.12s ´À %-12.12s    [%-40.40s] µû¤À [%s%d] %s",
...
board_edit ()
{
	...
	sprintf(buf, "¬ÝªO 1)¶R¤W­­ 2)¦WºÙ»Pµû¤À 3)¬ÝªO»¡©ú 4)¶iª©µe­± 5)¥i¨£¦W³æ 6)³]±K½X %s:",
		mode ? "7)¶i¶¥" : "");
	...
	switch(genbuf[0])
	{
	    ...
	    case '2':
		if(answer("¬ÝªO¬O§_¶}©ñµû¤À (a)¶}©ñ (c)Ãö³¬ ? ") == 'c')
...
struct one_key read_comms[] =
{
    ...
    'm', mark,            /* Mark ¤å³¹ */
    'X', refusemark,      /* ¤å³¹¥[±K */
    'L', editfriend,				/* ½s¿è¥[±K¤å³¹ªº¦n¤Í¦W³æ*/
