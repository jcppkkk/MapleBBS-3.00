  ...
  char *mode;
{
  ...
  if (count_multi() == 1)     //¨Ï¥ÎªÌmulti¤£ºâ®É¼Æ
...
*/
  ...
  ran = rand()%seed;
  if(ran<=50)                      
     show_file("etc/Welcome_title",0,21,ONLY_COLOR);
  else if(ran<=75)
     show_file("etc/Welcome_title",0,21,ONLY_COLOR);
  else
     show_file("etc/Welcome_title",0,21,ONLY_COLOR); 
...
//  counter(BBSHOME"/log/counter/¤W¯¸¤H¦¸","¥úÁ{¥»¯¸",1);
  show_file("etc/Welcome_news",21,0,ONLY_COLOR);
  ...
  while (1)
  {
    ...
    getdata(17, 0, " [31;47mùø ¢x    ¢x    ¢x   ¢x¢x¢¢¢wùð¢£¢x¢xùø[m     [±zªº±b¸¹] ",uid, IDLEN + 1 , DOECHO,0);
//    outs(" [31;47mùø ¢x    ¢x    ¢x   ¢x¢x¢¢¢wùð¢£¢x¢xùø[m");
//    getdata(17,49," [±zªº±b¸¹] ",uid, IDLEN + 1 , DOECHO,0);
    ...
    {
      getdata(19, 44, "[±zªº±K½X] ", passbuf, PASSLEN, PASS,0);
      passbuf[8] = '\0';
...
/* ªO¥D´£°ª«H½c¤W­­ */
  ...
  else if (!strcmp(cuser.userid, STR_GUEST))
  {
    ...
    char *name[10] = {"¥d±þ¦Ì¨È", "¥Õ³¾¿AÄR¤l", "µL¼Æ¤£¾Ç¤p¯¸", "¨gx©Î«Ox±¶", 
  ...
  cuser.uflag &= ~FRIEND_FLAG ; /* ­×¥¿¦n¤Í¦Cªí°ÝÃD */
}
