...
//  if (HAS_PERM(PERM_SYSOP) || !strcmp(muser.userid, cuser.userid)) 
  ...
  if (muser.exp < 50)             q=0;
    ...
    if (uentp && !(PERM_HIDE(currutmp) ||
      ...
      prints("[¥Ø«e°ÊºA][m¤£¦b¯¸¤W  [m\n");
    else
      prints("[¥Ø«e°ÊºA][m%-30.30s[m\n",
...
//    prints("[ Â¾  ·~ ]%-30.30s",racename[rpguser.race]);
    prints("[ ¶¥  ¯Å ][1;36m%-30.30s[m",explv[q]);
    prints("[¸gÀÙª¬ªp]%s\n",money[i]);
    if (HAS_PERM(PERM_SYSOP) || !strcmp(muser.userid, cuser.userid))
      prints("[ª÷¹ô¼Æ¶q]%-30ld[»È¹ô¼Æ¶q]%-21ld\n[ ¸gÅç­È ]%-30ld[»È¹ô¤W­­]%ld\n\n"
             ,muser.goldmoney,muser.silvermoney,muser.exp,MAXMONEY);

//    prints("[¤­¤l´Ñ³Ó/±Ñ/©M] %d/%d/%d",muser.five_win,muser.five_lost,muser.five_draw);
      prints("[¨Ï¥ÎªÌÅv­­]%-28d",muser.userlevel);
      prints("[¤W½u®É¼Æ]%d\n",muser.totaltime);   
...
//                     "¤p´I¯Î","¤¤´I¯Î","¤j´I¯Î","´I¥i¼Ä°ê","¤ñº¸¤¢ÀE"};
  ...
  if (tuid = getuser(uident))
  {
    ...
    j = muser.silvermoney + (10000 * muser.goldmoney);
    for(i=0;i<10 && j>10;i++) j /= 100;
    prints("[1;32m¡i­^¤å¥NºÙ¡j[m%-16.16s[1;32m¡i¼ÊºÙ¡j[m%-20.20s",muser.userid,muser.username);
//    prints("[1;32m¡i¦¸¼Æ¡j[m%s \n",racename[rpguser.race]);
    prints(" [1;32m¡i¦¸¼Æ¡j[m%d ¦¸\n",muser.numlogins);
                                                                                  
    prints("[1;32m¡i¸gÀÙª¬ªp¡j[m%-16s",money[i]);
    prints("[1;32m¡i¤ß±¡¡j[m%-21.21s",muser.feeling);
        ...
        || !strcmp(muser.userid, cuser.userid) )
    {
      char *sex[8] = { MSG_BIG_BOY, MSG_BIG_GIRL,
                       MSG_LITTLE_BOY, MSG_LITTLE_GIRL,
                       MSG_MAN, MSG_WOMAN, MSG_PLANT, MSG_MIME };
      prints("[1;32m¡i©Ê§O¡j[m%-14.14s\n",sex[muser.sex%8]);
    }
    else prints("\n");
                                                                                  
    prints("[1;32m¡i¤W¯¸¦aÂI¡j[m%-16.16s",muser.lasthost[0] ? muser.lasthost : "(LocalHost)");
    prints("[1;32m¡i®É¶¡¡j[m%-21.21s",Cdate(&muser.lastlogin));
//    prints("[1;32m¡iÂ¾·~¡j[m%s\n",racename[rpguser.race]);
    if (HAS_PERM(PERM_SYSOP) || !strcmp(muser.userid, cuser.userid))
    prints("[1;32m¡iª÷¹ô¡j[m%d \n",muser.goldmoney);
    else prints("\n");
    ...
    if (uentp && !(PERM_HIDE(currutmp) ||
      is_rejected(uentp) & HRM && is_friend(uentp) & 2)
      && PERM_HIDE(uentp))
      prints("[1;32m¡i¥Ø«e°ÊºA¡j[m¤£¦b¯¸¤W[m        ");
    else
      prints("[1;32m¡i¥Ø«e°ÊºA¡j[m%-16.16s[m",
...
//    outs(chkmail(1) ? "[1;5;33m¦³[m                   " : "[1;30mµL[m                   "); 
    chkmail(1);
//    prints("[5;58H[1;32m¡i¤å³¹¡j[m%d ½g\n",muser.numposts);
    if (HAS_PERM(PERM_SYSOP) || !strcmp(muser.userid, cuser.userid)) 
      prints("[5;58H[1;32m¡i»È¹ô¡j[m%d \n",muser.silvermoney);
    else prints("\n");

//    if (HAS_PERM(PERM_SYSOP) || !strcmp(muser.userid, cuser.userid))
//      prints("[1;32m¡iª÷¹ô¼Æ¶q¡j[m%-16ld[1;32m¡i»È¹ô¡j[m%-21ld[1;32m¡i¸gÅç¡j[m%ld\n"
//             ,muser.goldmoney,muser.silvermoney,muser.exp);
//    else prints("\n");
    prints("[1;32m¡i¤å³¹¼Æ¶q¡j[m%-16ld[1;32m¡i¶¥¯Å¡j[1;36m%-21ls"
             ,muser.numposts,racename[rpguser.race]);
//	     ,muser.numposts,explv[q]);
//    if (HAS_PERM(PERM_SYSOP) || !strcmp(muser.userid, cuser.userid))
//      prints("[1;32m¡i¸gÅç¡j[m%ld\n",muser.exp);
//    else prints("\n");
    prints("\n"); 
    
    prints("[1;32m¡i¤ô²y¡j[m¦¬%d /µo%d[K ",muser.receivemsg,muser.sendmsg);
    prints("[7;29H[1;32m¡i¤H®ð¡j[m%d[K ",muser.bequery);
    prints("[7;58H[1;32m¡i¦n©_¡j[m%d[K\n",muser.toquery);
    prints("[1;32m¡i­Ó¤H¦W¤ù¡j[m[K\n");
//    if(HAS_PERM(PERM_SYSOP))
...
" [1;34m¢w¢w¢w¢w¢w¢w¢w[37m¤ô[34m¢w[37m²y[34m¢w[37m¦^[34m¢w[37mÅU[34m¢w¢w¢w¢w¢w¢w¢w¢w¢w"COLOR1" [Ctrl-R]©¹¤U¤Á´« [34;40m¢w¢w¢w¢w¢w¢w [m");
         for(i=0 ; i < oldmsg_count ;i++)
          ...
          outs(
" [1;34m¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w"COLOR1" [Ctrl-T]©¹¤W¤Á´« [40;34m¢w¢w¢w¢w¢w¢w[m[K ");
     }
   ...
   int fd;
{
   ...
   sprintf(mid_line, COLOR2"  µL¼Æ¹ï¸Ü  [1m"COLOR1"%s%s¡j [37m»P  "COLOR1"%s%s[m",
...
//     if (myword / (now - talkstart) > 2) 
     if (exp > 1000)
       ...
       exp = 0;
     inexp(exp);
//     pressanykey("§A¥´¤F %d ¦r,¹ï¤è %d ¦r,±o¨ì %d ÂI¸gÅç­È¡C",myword,itword,exp);
     pressanykey("§A¥´¤F %d ¦r,¹ï¤è %d ¦r¡C",myword,itword);
     sprintf(buf,"%s»P%s²á¤Ñ, %d ¬í, %d ¦r, %d exp %s",
  ...
  user_info *uin;
{
  ...
  else
  {
    ...
    {
      pkmode = 1;
      getdata(2, 0, "½T©w­n©M¥L/¦o PK ¶Ü(Y/N)?[N] ", genbuf, 4, LCECHO,0);
    }
    else
    {
      showplans2(uin->userid);
      getdata(2, 0, "§ä¥L [y]²á¤Ñ [f]¤U¤­¤l´Ñ?  [N] ", genbuf, 4, LCECHO,0);
    }
...
pickup_user()
{
  ...
  while (1)
  {
    ...
    if (state >= US_ACTION)
    {
      ...
      prints("[200;442;506m[¡ö]¤W¤@­¶ [200;442;500;612m[Up[201m/[200;442;501;612mDn][201m [200;442;609m±Æ§Ç[%s][201;1;32m§ÚªºªB¤Í¡G%-3d "
        "[33m»P§Ú¬°¤Í¡G%-3d [36mªO¤Í¡G%-3d [31mÃa¤H¡G%-3d[m\n"
        COLOR1"[1m  %s P ¥N¸¹ [200;442;504;612m[¡ô[200;442;505;612m¡õ][201m  %-17s%-17s%-13s%-10s[m\n",
...
//      strcpy(cuser.realname, uentp->realname);
    ...
    move(b_lines, 0);
    outs("[1;44;33;200;442;716m(t)[37m¸ò¥L¡þ¦o²á¤Ñ  \
[33;200;442;711m(o)[37m¥æ¤Í¦W³æ  [33;200;442;709m(m)[37m±H«H  \
[33;200;442;714m(r)[37m¶i¤J«H½c  [33;200;442;708m(l)[37m¤ô²y¦^ÅU   [33;200;442;619m[^S][200;442;712m[p][200;442;705m[i][201m   [m\
");
    state = 0;
    while (!state)
    {
      ...
      case 'E':
        ...
        {
          char ans[2];
          getdata(b_lines-1, 0,"±Æ§Ç¤è¦¡ [200;442;649;613m1.¦n¤Í [200;442;650;613m2.¥N¸¹ [200;442;651;613m3.°ÊºA [200;442;652;613m4.µo§b [200;442;653;613m5.¬G¶m [200;442;654;613m6.¬ÝªO [201m"
            ,ans, 2, LCECHO,"");
            if(!ans[0])
            {
              state = US_REDRAW;
  ...
  if (currutmp->destuid == 6)
   {
    if (HAS_PERM(PERM_SYSOP))
    {
      if (!cuser.userlevel || !getdata(b_lines, 0, "µo§bªº²z¥Ñ¡G", currutmp->chatid, 13, DOECHO, 0))
      currutmp->destuid = 0;
    }
    else
    {
...
talkreply()
{
  ...
  clear();
  outs("\n [1;36m¡º[1;32m¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w¢w[1;36m¡º[m
  ...
  prints("   [1;37m¹ï¤è¨Ó¦Û [%s]¡A¦@¤W¯¸ %d ¦¸¡A¤å³¹ %d ½g[m\n",
