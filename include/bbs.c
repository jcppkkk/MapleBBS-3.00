...
void
score_note(prompt, fhdr, direct, choose)        //±ÀÂË¥[¤À!! ¾Ç ptt by hialan
  fileheader * fhdr;
  char *direct, *prompt;
  int choose ;
{
    ...
    if ( choose == 1 ) {                                                                           
      ...
      fprintf(fp, "\n\033[1;31m¡÷ \033[33m%s\033[0;33m:%-*s\033[1;32m¦R \033[m%14.14s %0d/%0d",
                  cuser.userid, 51-strlen(cuser.userid), genbuf, fromhost,
                  ptime->tm_mon+1, ptime->tm_mday);
    }  
    flock(fd, LOCK_UN);
...
score (ent, fhdr, direct)
  ...
  char *direct;
{
  char buf[256] , needscore[4] , path[128] ;
  int n , sklscore, sklscore1, CompareId;
  time_t now = time(0);
  ...
  if (currstat == RMAIL ||
     ((cuser.scoretimes <= 0                                     // ¨S¦¸¼Æ
       || currbrdattr &  ( BRD_PERSONAL || BRD_HIDE)) && !HAS_PERM(PERM_SYSOP)) // ­Ó¤HªO¤S
     || !strcmp("guest",cuser.userid) || !HAS_PERM(PERM_POST))                           // ¤£¬O¯¸ªø
  ...
  getdata (b_lines, 0, "½Ð°Ý­n [1]¥[¤À [2]¦©¤À [3]±ÀÂË [4]¦R¼Ñ : ", buf, 2, LCECHO, 0);
  	if(!buf[0] || buf[0] < '1' || buf[0] > '4') {
    	  sprintf(buf , "%s!!", "¨ú®ø");
    	    pressanykey(buf);
    	    return RC_DRAW;
  	} 
	    	...
	    	if ( fhdr->score == 9 ) {
	    	  sprintf(buf , "¤w¸g¬O³Ì%s¤À¤F!!", fhdr->score >= 9 ? "°ª" : "§C");
    			...
    			if ( HAS_PERM(PERM_SYSOP)) {
    				...
    				if ( fhdr->score > 9 )
    					fhdr->score = 9 ;
    				score_note("±ÀÂË¤º®e(¤£¿é¤J¸õ¹L): ", fhdr, direct, 1);
  	...
  	if ((HAS_PERM(PERM_SYSOP)) || (CompareId !=0)){
	    	if ( fhdr->score == -9 ) {
   			...
   			if ( HAS_PERM(PERM_SYSOP)) {
   				...
   				if ( fhdr->score < -9 )
   					fhdr->score = -9 ;
   				score_note("¦R¼Ñ¤º®e(¤£¿é¤J¸õ¹L): ", fhdr, direct, 2);
  	...
  	{
    	score_note("±ÀÂË¤º®e: ", fhdr, direct, 1);
    	return RC_DRAW;
  	}
  
  	else if(buf[0] == '4')        // ±ÀÂË ....:) ¾Ç ptt by hialan
  	{
    	score_note("¦R¼Ñ¤º®e: ", fhdr, direct, 2);
    	return RC_DRAW;
  	}
  ...
  getdata (b_lines - 2, 0, "¯d¨¥¸É¥R¡H", path, 45, LCECHO, 0);
  if(path[0])
  {
    time_t now = time(0);
    ...
    struct tm *ptime = localtime(&now);
    sprintf(buf,
    "¡Ð¡Ö \033[1;%s \033[0;37m%s \033[m\033[33m: %s\033[m%*s %02d/%02d %02d:%02d",
      n == 1 ? "31mÆg" : n == 2 ? "32mÄê" : "37m¡·", cuser.userid, path,
      55 - strlen(cuser.userid) - strlen(path), "-",
      ptime->tm_mon + 1, ptime->tm_mday, ptime->tm_hour, ptime->tm_min);
    sprintf(path,"%s/boards/%s/%s",BBSHOME, currboard, fhdr->filename);
    sprintf(path,"%s/boards/%s/%s",BBSHOME, currboard, fhdr->filename);
    f_cat(path,buf);
  }
...
board_edit ()
{
  ...
  if (currmode & MODE_BOARD)
  {
    ...
    sprintf(buf, "¬ÝªO 1)¶R¤W­­ 2)§ï¤¤¤å¦WºÙ 3)¬ÝªO»¡©ú 4)¶iª©µe­± 5)¥i¨£¦W³æ 6)³]±K½X %s:",
      mode ? "7)¶i¶¥" : "");
