...
{
  char buf[128];
  sprintf(buf, "/usr/bin/gzip -9n adm/%s%s", target, stamp);
  f_mv(source, &buf[14]);
  system(buf);
}
...
main()
{
  ...
  if (!ptime->tm_hour)
    keeplog("etc/today", "Record", "[µL¼Æ¬ö¿ý] ¤W¯¸¤H¦¸²Î­p");
  ...
  now += ADJUST_M * 60;         /* back to future */
   ...
   /* Ptt ¾ú¥v¨Æ¥ó³B²z */
    if(fp = fopen("etc/history.data","r"))
	 ...
	 int a;
         resolve_fcache();
         printf("¦¹®É¬q³Ì¦h¦P®É¤W½u:%d ¹L¥h:%d", fcache->max_user,k);
	 a=fcache->max_user;
         fclose(fp);
         if(a > k)
                {
...
if(!ptime->tm_hour)
  {
    keeplog(BBSHOME"/.note", "Record", "[µL¼Æ¬ö¿ý] ¤ß±¡¯d¨¥ª©");
    ...
    keeplog(BBSHOME"/log/article_score.log", "Security", "[¨t²Î³ø§i] ¤å³¹µû¤À¬ö¿ý");
...
*/
    if (ptime->tm_wday == 0)
    {
      keeplog("log/week", "Record", "[µL¼Æ¬ö¿ý] ¥»¶g¼öªù¸ÜÃD");
...
//      keeplog("log/bingo.log", "LocalGame","[¹C¼Ö³õ] ¥»¶g»«ªG¬ö¿ý");
    ...
    if (ptime->tm_mday == 1)
    {
      keeplog("log/month", "Record", "[µL¼Æ¬ö¿ý] ¥»¤ë¼öªù¸ÜÃD");
      keeplog("log/topboard", "Record", "[µL¼Æ¬ö¿ý] ¥»¤ë¬ÝªO±Æ¦æ");
      keeplog("log/toppersonal", "Record", "[µL¼Æ¬ö¿ý] ¥»¤ë­Ó¤HªO±Æ¦æ");
    }
    if (ptime->tm_yday == 1)
      keeplog("log/year", "Record", "[µL¼Æ¬ö¿ý] ¦~«×¼öªù¸ÜÃD");
  }
