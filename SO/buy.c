...
p_fcloak()
{
  ...
  if (getans("½T©w­nªá $10000 ¾Ç²ß²×·¥Áô¨­¤jªk¡H[y/N]") != 'y')
    return;
...
p_ffrom()
{
  ...
  if (getans("½T©w­nªá $50000 ÁÊ¶R­×§ï¬G¶mÄ_¨å¡H[y/N]") != 'y')
     return;
  rec_get(fn_passwd, &xuser, sizeof(xuser), usernum);
...
p_ulmail()
{
  ...
  if (getans("½T©w­nªá $5000000 ÁÊ¶RµL¤W­­«H½c?[y/N]") != 'y')
    return;
  rec_get(fn_passwd, &xuser, sizeof(xuser), usernum);
...
//  counter(BBSHOME"/log/counter/»È¦æ","¨Ï¥Î»È¦æ",0);
  counter(BBSHOME"/log/counter/»È¦æ","¨Ï¥Î",0);
  move(2, 0);
  update_data();
