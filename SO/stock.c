...
//	 more("game/stock_wel",YEA);
	...
	if(ff=fopen(BBSHOME"/game/stock_now","r"))
	{
	 ...
	 if(strcmp(tmp,sto->ud))
	 {
	  ...
	  game_log(STOCK,"[41m§ó·s¸ê®Æ [42m µL¼ÆªÑ¥« %s",sto->ud);
	...
	/* µe­±¥Dµ{¦¡ */
	 ...
	 move(22,0);
	 prints("[44mn:¤U­¶ p:¤W­¶ b:¶R s:½æ v:¬Ý e:­× q:¸õ¥X »È:%-8d %2d/%2d %18s[m",
...
//	  cuser.silvermoney,page+1,sto->max_page);
	  ...
	  case 'b':
	    ...
	    {
//	      pressanykey("¥æ©ö®É¶¡¬°¨C¤Ñ 3:00pm ¨ì¹j¤Ñ¦­¤W 8:00am .. :)");
	      pressanykey("3:00pm~4:00pm ªÑ¥«§ó·s¤¤ °±¤î¥æ©ö .. :)");
	      break;
	    }
	    ...
	    game_log(STOCK,"[32m¶R¤F [44m%s [m[43m%d ±i [1;31m ¤âÄò¶O %d", 
	      sto->name[tt-1],t1, td);

	    sprintf(tp2,BBSHOME"/home/%s/stock_log",cuser.userid);
	    fff=fopen(tp2,"a+");
	//    td=t1*pric*0.07;
	    fprintf(fff,"%s ¥H %5.2f ªº»ù®æ¶R¤F %s %d ±i ¤âÄò¶O %d\n", 
...
//	    pressanykey("¥æ©ö®É¶¡¬°¨C¤Ñ 3:00pm ¨ì¹j¤Ñ¦­¤W 8:00am .. :)");
    ...
    now=time(0);
    td=t1*pric*0.01425;   
    game_log(STOCK,"[31m½æ¤F [44m%s [m[43m%d ±i [1;31m ¤âÄò¶O %d", 
      sto->name[tt-1],t1, td);
    td=t1*pric*0.01425;
    tax=t1*pric*0.03;
    sprintf(tp2,BBSHOME"/home/%s/stock_log",cuser.userid);
    fff=fopen(tp2,"a+");
    fprintf(fff,"%s ¥H %5.2f ªº»ù®æ½æ¤F %s %d ±i ¤âÄò¶O %d µ| %d\n", 
