...
/* ¿é¤JÃ±¸@¸¹½X */
   do{
     strcpy(buf,"");
//     move(10,0);
//     prints("¤p¯µ³Z:[1;35;40m ¿é¤J³æ¤@­Ó\"c\", ¥ÑµL¼Æ¹q¸£À°±z¿ï¸¹£¬![m");
...
/*Åª¨úget_lotto_now.txtªº¸ê®Æ*/
   ...
   while(i){
     ...
     prints("½Ð¦b¨C­Ó¬P´Á¤G,¤­ªº7:30pm~9:00pm ¶}¼ú¥H¥~ªº®É¶¡Ã±½ä");
     move(7,0);
     prints("±z¥Ø«e¦@¦³ %d ª÷¹ô, ¥ç¤wÃ±¤FµL¼Æ¼Ö³z%d°¦",cuser.goldmoney,lotto_amount(cuser.userid));
     ...
     gamble_money=atol(buf);
//     move(10,0);
//     prints("ª÷¹ô[1;33;40m%ld[m¤¸/ª`....",gamble_money);
     if(strcmp("",buf)==0){
	return 0;
     }
...
//        continue;	
	return;
     ...
     strcpy(buf,"");
     move(10,0);
     prints("½Ðª`·N:[1;35;40m ±zªºª÷¹ô¬O§_¨¬°÷Åý±z¤U¨º»ò¦hª`[m");
...
//  if(! belong("lotto.usr",cuser.userid) )
//  {
//   pressanykey("¹ï¤£°_.±z¤£¦b´ú¸Õ¦W³æ¤º!!");
//   return ;
//  }
//´ú¸Õ¥Î                   
...
//  while(j==0){
   ...
   prints("[1;33;44m                           >>>>>µL¼Æ¼Ö³z±mÃ±½ä¯¸<<<<<                         [m\n");
   move(2,10);
//   prints("¥Ø«e¤w¸g¶}¼ú¨ì²Ä [1;33;40m%s[m ´Á, ©ó %s ¶}¼ú¤F!!",data_now_menu[0],data_now_menu[1]);
...
//   prints("[m   ¯S§O¸¹ : [1;35;40m%s[m",data_now_menu[8]);
   ...
   prints("¥Ø«e±z¤w¸gÃ±¤F%d¤äµP,³Ì¤j¥iÅý§AÃ±%d¤ä",lotto_amount(cuser.userid),LOTTO_AMOUNT_MAX);
	...
	move(k+4,i);
	prints("[0] Â÷¶}µL¼Æ¼Ö³zÃ±½ä¯¸(default)");
	strcpy(buf,"");
	getdata(k+6,i,"½Ð¿é¤J±z­n°õ¦æªº¶µ¥Ø:",buf,2,1,0);
